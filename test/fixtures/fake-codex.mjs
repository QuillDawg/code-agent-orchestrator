...
if (args[0] === 'login' && args[1] === 'status') { print('Logged in'); process.exit(0); }
...
if (args.includes('exec')) {
  ...
  await new Promise((resolve) => {
    ...
    process.stdin.on('end', () => {
      ...
      if (outputFlag >= 0 && args[outputFlag + 1]) writeFileSync(args[outputFlag + 1], JSON.stringify(execResult), 'utf8');
...
rl.on('line', (line) => {
  ...
  if (message.method === 'initialize') {
    ...
    emit({ id: message.id, result: {
      thread: { id: threadId }, model: message.params?.model ?? 'fake-codex', modelProvider: 'openai', cwd: message.params?.cwd ?? process.cwd(),
      instructionSources: [], approvalPolicy: message.params?.approvalPolicy ?? 'on-request', approvalsReviewer: message.params?.approvalsReviewer ?? 'user',
  ...
  } else if (message.id === 99 || message.id === 100) {
...
function finish() {
  emit({ method: 'item/completed', params: { threadId, turnId: 'turn-1', item: { type: 'agentMessage', id: 'msg-1', text: JSON.stringify(result), phase: 'final_answer', memoryCitation: null, delivery: null, questions: null } } });
