# 2.0.0-beta.1 — development log

The complete record of what changed on the way to
[2.0.0-beta.1](https://github.com/QuillDawg/code-agent-orchestrator/blob/main/CHANGELOG.md), kept out of
the shipped `CHANGELOG.md` because it is a work log rather than a release note: it follows individual
frame fixes, internal module renames and the reasoning behind decisions that a reader deciding whether to
upgrade does not need.

**For "should I upgrade, and what will move", read
[CHANGELOG.md](https://github.com/QuillDawg/code-agent-orchestrator/blob/main/CHANGELOG.md) instead.** This
file is for anyone who wants to know *why* something is the way it is.

It is not published to npm, and it is not a Keep a Changelog document — the section headings below are the
ones the work was grouped under while it was being done.
