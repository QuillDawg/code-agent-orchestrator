...
run, a run can be driven from a second terminal, tasks can be edited and talked to while the workflow is
still going, and `cao doctor` no longer spends anything unless you ask it to.

**No workflow YAML key was added, removed or renamed**, so a 0.1 workflow file runs unchanged. The number
jumped from the 0.1 line because this is a new generation rather than a patch on it.

The full development log for this release, with the reasoning behind each change, is in
[CHANGELOG-2.0.0-beta.1.md](CHANGELOG-2.0.0-beta.1.md).

### Added

- **A persistent workspace.** `cao run` and `cao resume` open a shell with a task list and six tabs —
  Overview, Session, Logs, Changes, Report, Diagnostics — which stays open when the run ends, whatever the
  outcome, with the reason and the resume actions on screen.
- **`cao ui [run]`** opens that workspace on any run: one this terminal owns, one another terminal is
  executing (as a read-only observer), or one that finished days ago.
- **A run can be driven from another terminal.** `cao task stop`, `cao task restart`, `cao task prompt` and
  `cao task edit` reach a running workflow through a request inbox the owning process answers.
- **Editing and prompting an unfinished task.** `cao task edit` changes a task's prompt, agent, model,
  effort, timeout, retries or budget; `cao task prompt` steers a running worker, stops and continues it, or
  starts a stopped one again carrying your message.
- **`cao diagnostics [run] --out <file>`** — one redacted JSON file describing a run, to attach to a bug
  report.
- **Seven new `cao doctor` checks**, each graded with its evidence and one line saying what to do about it.
- **A quota footer**: one chip per provider, refreshed from account reads that start no turn and bill
  nothing.
- **A second published package, `code-agent-orchestrator-protocol`** — the types, the run-directory layout
  and the transcript-planning logic a desktop surface needs, with no Node builtins and no dependencies.
- **`cao emit`** announces a run to a desktop app on the same machine. Off by default, and it writes nothing
  but a small file under `~/.cao`.
- **A theme.** `cyberpunk` by default and `mono` for a readable unpainted screen, selected by `--theme`,
  `CAO_THEME` or a `~/.cao/config.json` key; `NO_COLOR`, `TERM=dumb` and a screen reader are honoured.

### Changed

- **`cao doctor` starts no agent unless `--probe` asks it to.** The live probes used to be on by default, so
  the command that diagnoses a broken install cost a model call to run.
- **`cao` on its own prints its help to stdout and exits `0`**, where it used to print to stderr and exit
  `2`. Root help is grouped: Run, Inspect, Task controls, Diagnostics.
- **The workspace opens in the alternate screen** instead of wiping your scrollback. `Q` asks before it
  leaves, and `Ctrl+C` no longer closes it.
- **The default theme is named `cyberpunk`**, not `default`; `--theme default` and `CAO_THEME=default` are
  still accepted as the old name.
- **`needs_input` has a glyph of its own**, `!`, where it used to share `?` with `waiting`.
- **`--debug` and `CAO_DEBUG=1` turn the logger up to debug** and write an `orchestrator.log` worth reading.
  `CAO_DEBUG` used to affect only the stack trace printed when a command failed.
- **An announced run advertises what it accepts.** A registry entry's `capabilities` list was always `[]`.
- **The terminal UI runs on Ink 7 and React 19.** Backspace and Delete are no longer the same key: Backspace
  erases the character behind the cursor when you are typing a denial reason or a `/` search, and Delete no
  longer does.

### Fixed

- The workspace fits an 80x24 terminal: the sidebar becomes a one-line task strip, summaries are truncated
  rather than wrapped, and the frame no longer shifts sideways as focus moves.
- An attempt the orchestrator cancelled ends its transcript with the cancellation rather than stopping
  mid-sentence.
- `orchestrator.log` is written synchronously, so two lines written at once can no longer interleave.
