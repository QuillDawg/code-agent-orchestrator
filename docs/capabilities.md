...
`--theme mono` (or `CAO_THEME=mono`, or `NO_COLOR`) paints nothing at all, which is the check that every state is readable as a glyph plus a word rather than as a colour. `CAO_ASCII=1` draws the state marks, the spinner, the progress bars, the cursor and the tab separators in ASCII. `CAO_REDUCED_MOTION=1`, a screen reader, or `TERM=dumb` stop the spinner and mark a running task with its own glyph instead.
