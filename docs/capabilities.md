...
- [Structuring work](#structuring-work)
  - [Run tasks in order](#run-tasks-in-order)
  - [Run tasks in parallel](#run-tasks-in-parallel)
  - [Express an explicit dependency graph](#express-an-explicit-dependency-graph)
  - [Fan out over a list](#fan-out-over-a-list)
  - [Reuse a task shape](#reuse-a-task-shape)
- [Passing information between tasks](#passing-information-between-tasks)
  - [Give a task the results of earlier tasks](#give-a-task-the-results-of-earlier-tasks)
  - [Return machine-readable data](#return-machine-readable-data)
- [Controlling what runs](#controlling-what-runs)
  - [Skip a task unless a condition holds](#skip-a-task-unless-a-condition-holds)
  - [Pause for a human decision](#pause-for-a-human-decision)
  - [Answer a worker while it runs](#answer-a-worker-while-it-runs)
  - [Let a worker ask a question after it stopped](#let-a-worker-ask-a-question-after-it-stopped)
  - [Run only part of a workflow](#run-only-part-of-a-workflow)
- [Handling failure](#handling-failure)
  - [Retry with knowledge of the previous attempt](#retry-with-knowledge-of-the-previous-attempt)
  - [Decide what a failure costs](#decide-what-a-failure-costs)
  - [Survive transient API errors for free](#survive-transient-api-errors-for-free)
  - [Resume after a crash or Ctrl+C](#resume-after-a-crash-or-ctrlc)
  - [Editing an unfinished task](#editing-an-unfinished-task)
  - [Prompting a task](#prompting-a-task)
- [Isolation and git](#isolation-and-git)
  - [Keep parallel agents out of each other's way](#keep-parallel-agents-out-of-each-others-way)
  - [Work in a subdirectory](#work-in-a-subdirectory)
  - [Opt out of git entirely](#opt-out-of-git-entirely)
- [Choosing the agent](#choosing-the-agent)
  - [Constrain what an agent may do](#constrain-what-an-agent-may-do)
- [Running your own commands](#running-your-own-commands)
- [The workspace](#the-workspace)
  - [The run ends and the workspace stays](#the-run-ends-and-the-workspace-stays)
  - [Watching a run another terminal owns](#watching-a-run-another-terminal-owns)
  - [Terminals](#terminals)
  - [Provider quotas in the footer](#provider-quotas-in-the-footer)
  - [Finding things in a long transcript](#finding-things-in-a-long-transcript)
  - [Attempt and interaction history](#attempt-and-interaction-history)
- [Reviewing what a task changed](#reviewing-what-a-task-changed)
  - [In the dashboard](#in-the-dashboard)
- [Reporting a run](#reporting-a-run)
- [Reusing results across runs](#reusing-results-across-runs)
- [Secrets](#secrets)
- [Finding the right command](#finding-the-right-command)
- [Checking the installation](#checking-the-installation)
  - [`--probe` starts the modes for real](#--probe-starts-the-modes-for-real)
  - [A bug report in one file](#a-bug-report-in-one-file)
- [Where the truth lives](#where-the-truth-lives)
  - [Beyond the repository: `~/.cao/`](#beyond-the-repository-cao)
...
The workspace has two themes. **`cyberpunk`** is the default: violet and cyan accents over a dark violet chrome, one warning colour and one danger colour, panel borders in the accent, and a compact `cao <run id>` wordmark on the header's first line — three letters and the run, not banner art. The colours are written as hex in one token table (`src/tui/theme.ts`, the only file under `src/tui/` allowed to name a colour, which a test enforces) and are downsampled for you: a truecolor terminal gets the shade that was chosen, a 256-colour one the nearest cell of the cube, a 16-colour one the nearest of the sixteen.

**`--theme mono`** (or `CAO_THEME=mono`, or `NO_COLOR`, or a terminal that reports no colour at all) paints no colour and has only bold, dim and inverse. That is the check that every state, badge and delivery state is readable as a glyph plus a word rather than as a colour: focus is a `▸` marker and a doubled panel border, the open tab is in brackets, and a selected row is inverse. The theme is chosen in the usual order — `--theme` beats `CAO_THEME`, which beats a `"theme"` key in `~/.cao/config.json` (read, never written, exactly as `altScreen` is), which beats the default — except that `NO_COLOR` beats all four. `--theme default` still means `cyberpunk`, which is what it named before the palette had one.

`CAO_ASCII=1` draws the whole workspace in ASCII: the state marks, the spinner, the progress bars, the cursor and the tab separators, the focus marker (`>`) and the activity pulse (`*`), and also the parts that used to be spelled out in Unicode wherever they were written — the arrows a key is named after (`^v` for `↑↓`, `<>` for `←→`), the header's separator, the `->` notes under an attempt, the `+/-` file counter, and the bullets and rules of the rendered report.

Two things move, and nothing else does: the spinner on a running task, and a two-frame pulse in the sidebar gutter of a task that has produced output in the last second. `CAO_REDUCED_MOTION=1`, a screen reader, or `TERM=dumb` stop both and mark a running task with its own glyph instead. When Ink reports a screen reader the header and the footer each collapse to a single line and the lists announce their position as `3 of 12` rather than `3/12`, so a reader is not read three rows of chrome before the row that changed.
