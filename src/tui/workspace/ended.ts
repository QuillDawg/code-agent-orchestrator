...
export function endedActions(run: WorkflowRun, selected: ResolvedTask | undefined): EndedAction[] {
  const actions: EndedAction[] = [{ key: 'S', label: 'Resume run', kind: 'resume' }];
  if (!selected) return actions;
  const state = run.tasks[selected.id]?.state;
  actions.push({ key: 'R', label: `Re-run ${selected.id}`, kind: 'task', taskId: selected.id });
  actions.push({ key: '>', label: `Resume from ${selected.id}`, kind: 'from', taskId: selected.id });
  if (state === 'needs_input') actions.push({ key: 'A', label: `Answer ${selected.id} and resume`, kind: 'answer', taskId: selected.id });
  if (state === 'awaiting_approval') {
    actions.push({ key: 'A', label: `Approve ${selected.id}`, kind: 'approve', taskId: selected.id });
    actions.push({ key: 'X', label: `Reject ${selected.id}`, kind: 'reject', taskId: selected.id });
  }
  return actions;
}
